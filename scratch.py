...
for _ in range(EPOCHS):
    ...
    for i, batch_id in enumerate(dataset_bar):
        ...
        actor_loss = (torch.stack([a*l for a,l in zip(advantages.detach(),action_log_probs)]).mean().sum() * -1)
        # The critic wants to match the actual rewards as much as possible
        # So it's just MSE loss 
        critic_loss = advantages.pow(2).mean()

        # Add the losses
        loss = (actor_loss+critic_loss)/REPLAY
        ...
        torch.nn.utils.clip_grad_norm_(critic_model.parameters(), 0.1)
